...
export interface INotesClient {
    getAll(): Observable<NotesListViewModel | null>;
}
...
export class NotesClient implements INotesClient {
    ...
    getAll(): Observable<NotesListViewModel | null> {
        let url_ = this.baseUrl + "/api/Notes/GetAll";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return <Observable<NotesListViewModel | null>><any>_observableThrow(e);
                }
            } else
                return <Observable<NotesListViewModel | null>><any>_observableThrow(response_);
        }));
    }

    protected processGetAll(response: HttpResponseBase): Observable<NotesListViewModel | null> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = resultData200 ? NotesListViewModel.fromJS(resultData200) : <any>null;
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
        ...
        return _observableOf<NotesListViewModel | null>(<any>null);
    }
}
...
export class NotesListViewModel implements INotesListViewModel {
    notes?: NoteLookupModel[] | undefined;

    constructor(data?: INotesListViewModel) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
        ...
        if (data) {
            if (data["notes"] && data["notes"].constructor === Array) {
                ...
                for (let item of data["notes"])
                    this.notes!.push(NoteLookupModel.fromJS(item));
            }
        }
    }

    static fromJS(data: any): NotesListViewModel {
        data = typeof data === 'object' ? data : {};
        let result = new NotesListViewModel();
        result.init(data);
        return result;
    }
...
export interface INotesListViewModel {
    notes?: NoteLookupModel[] | undefined;
}

export class NoteLookupModel implements INoteLookupModel {
    id?: number;
    titre?: string | undefined;
    description?: string | undefined;

    constructor(data?: INoteLookupModel) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): NoteLookupModel {
        data = typeof data === 'object' ? data : {};
        let result = new NoteLookupModel();
        result.init(data);
        return result;
    }
...
export interface INoteLookupModel {
    id?: number;
    titre?: string | undefined;
    description?: string | undefined;
}
